...
W = tb.core.Variable(dim=(3,4), init=True, trainable=True)

b = tb.core.Variable(dim=(3, 1), init=True, trainable=True)

# 线性部分
linear = tb.ops.Add(tb.ops.MatMul(W, x), b)

# 模型输出
predict = tb.ops.SoftMax(linear)

# 交叉熵损失
loss = tb.ops.loss.CrossEntropyWithSoftMax(linear, one_hot)
